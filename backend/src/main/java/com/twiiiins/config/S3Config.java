...
public class S3Config {
    ...
    public String s3ConfigInitializer() {
        ...
        log.info("Secret Access Key: {}",
                (secretAccessKey != null && !secretAccessKey.isEmpty() ? "✅ 설정됨" : "❌ 설정되지 않음"));
        log.info("Endpoint: {}", (endpoint != null && !endpoint.isEmpty() ? endpoint : "AWS S3 기본값"));
        log.info("========================");
    ...
    public S3Client s3Client() {
        ...
        if (endpoint != null && !endpoint.isEmpty()) {
            log.info("커스텀 Endpoint를 사용합니다: {}", endpoint);
