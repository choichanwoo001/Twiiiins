...
export const musicService = {
  ...
  async searchMusic(filters) {
    ...
    const response = await axios.get(`/media/music?${params.toString()}`)
    return response.data.data || response.data
  },

  // 음악 생성
  async createMusic(musicData) {
    const response = await axios.post('/media/music', musicData)
    return response.data.data || response.data
  },

  // 음악 수정
  async updateMusic(id, musicData) {
    const response = await axios.put(`/media/music/${id}`, musicData)
    return response.data.data || response.data
  },
